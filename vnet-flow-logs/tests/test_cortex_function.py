"""
E2E tests for vnet-flow-logs CortexFunction.
...
# Add parent directory to path to import CortexFunction
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))
from CortexFunction import denormalize_vnet_records, main
                        ...
                        "flowGroups": [
                            {
                                "rule": "DefaultRule",
                                "flowTuples": [
                                    "1705311600,10.0.2.20,20.30.40.70,9999,22,T,I,A"
                                ],
                            }
                        ]
                    }
...
def mock_env():
    ...
    with patch.dict(
        ...
        import CortexFunction

        importlib.reload(CortexFunction)
        yield
...
def captured_requests():
    ...
    with patch("CortexFunction.requests.post", side_effect=mock_post):
        yield requests


class TestCortexFunctionE2E:
    """End-to-end tests for the CortexFunction"""

    def test_process_vnet_flow_log_v2_all_records_received(
        ...
        assert len(all_received_records) == 4, (
            f"Expected 4 records, got {len(all_received_records)}"
        )
    ...
    def test_whitespace_only_blob_no_requests(self, mock_env, captured_requests):
        ...
        # Assert
        assert len(captured_requests) == 0, (
            "No requests should be sent for whitespace-only blob"
        )
    ...
    def test_partial_json_no_requests(self, mock_env, captured_requests):
        ...
        assert len(captured_requests) == 0, (
            "No requests should be sent for invalid JSON"
        )
    ...
    def test_empty_records_array_no_requests(self, mock_env, captured_requests):
        ...
        # Assert
        assert len(captured_requests) == 0, (
            "No requests should be sent for empty records"
        )
    ...
    def test_missing_endpoint_no_requests(self, captured_requests, mock_env):
        ...
        with patch.dict(os.environ, {"CORTEX_ACCESS_TOKEN": "test-token"}, clear=True):
            ...
            # Act
            CortexFunction.main(mock_blob)

            # Assert
            assert len(captured_requests) == 0, (
                "No requests should be sent without endpoint"
            )
    ...
    def test_missing_token_no_requests(self, captured_requests, mock_env):
        ...
        # Arrange
        with patch.dict(
            os.environ, {"CORTEX_HTTP_ENDPOINT": "https://test.example.com"}, clear=True
        ):
            import importlib
            ...
            # Assert
            assert len(captured_requests) == 0, (
                "No requests should be sent without token"
            )
    ...
    def test_large_payload_batching(self, captured_requests):
        ...
        large_dataset = {
            "records": [
                ...
                for i in range(
                    100
                )  # 100 records with 10 tuples each = 1000 total denormalized records
            ]
        }
        ...
        assert len(all_received_records) == 1000, (
            f"Expected 1000 records, got {len(all_received_records)}"
        )
