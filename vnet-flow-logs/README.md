...
This repository contains an Azure Function that collects VNET Flow Logs from Azure and forwards them to Cortex. The Azure Function is deployed using an ARM template that creates all the necessary Azure resources such as private storage account (for internal usage), private endpoints, subnets.
...
#### Required Steps:
* In the Azure Portal, go to your Storage Account > Security + networking > Networking.
* Locate the Virtual networks section.
* Click + Add existing virtual network.
* Select the VNET and Subnet created during the Cortex Flow Logs Collector deployment.
