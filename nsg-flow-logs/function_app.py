...
CHECKPOINT_TABLE_NAME = os.environ.get('CHECKPOINT_TABLE_NAME', 'nsgflowcheckpoints')
...
logging.info('Registering nsg_flow_log_trigger...')


@app.blob_trigger(
    arg_name='blob', path='insights-logs-networksecuritygroupflowevent/{name}', connection='TargetAccountConnection'
)
def nsg_flow_log_trigger(blob: func.InputStream):
        ...
        for outer_flow in record['properties']['flows']:
            for inner_flow in outer_flow['flows']:
                for flow_tuple in inner_flow['flowTuples']:
                    denormalized_record = create_nsg_record(record, outer_flow, inner_flow, flow_tuple)
...
def create_nsg_record(record, outer_flow, inner_flow, flow_tuple):
